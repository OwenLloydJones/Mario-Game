...
local motionY = 0
local score   = 0
local timeStart = 1 --< the length of the game
local timeLeft
local level = 1
...
function finalAlert( )
	...
	-- and score
	alert = display.newImage('AlertBg.png', 360, 205)

	transition.from(alert, {time = 300, xScale = 0.3, yScale = 0.3})
	finalScore = display.newText(score.." pts" , 200, 180, native.systemFontBold, 28)
...
function onCollision(self,event)
	-- Something has collided with the monkey
	if ( event.phase == "began" ) then
	
		-- Check to see if the object is worth points
		-- and should have been collected
		if self.points <= 0 then
			ouch=display.newImage("marioOuch.png", monkey.x, monkey.y)
			transition.dissolve(ouch,monkey, 1000, 0)
			audio.play(hitSound)
			else
			audio.play(yesSound)
		end
...
function newItem( )
	...
	local points = {-1, 3, 5, 1, -1, 0, -1, -1 ,-1 ,math.random(-5,5), math.random(0,10)}
...
function moveMonkey (event)
	-- Move the monkey each time a frame is triggered
	-- Use the Y value due to the rotated screen
	monkey.x = monkey.x + motionY
	monkey.y = monkey.y + motionX
	wrap ( )
end

function wrap (event)
	-- Stop the monkey at the edge
	if monkey.x < 30 then
		monkey.x = 30
	elseif monkey.x > ScreenW-30 then
		monkey.x = ScreenW-30
	end
	if monkey.y > 270 then
		monkey.y = 270
	elseif monkey.y < 70 then
		monkey.y = 70
	end
end

function left( )
	-- move the monkey image to the left
	motionY = -10
end

function right( )
	-- move the monkey image to the right
	motionY = 10
end

function stop(event)
	-- Stop the monkey from constantly moving
...
function drawBackground( )
	...
	-- Display the monkey
	monkey = display.newImage ("mario.png", 210, 270)
	physics.addBody( monkey,"static",{radius=30})
	-- Display arrows
